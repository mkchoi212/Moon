...
final class WalletModel: ObservableObject {
    @AppStorage("current.wallet") var currentWallet: Wallet?
    ...
    init() {
        ...
        qrGenerator.currentWalletAddress = currentWallet?.address ?? ""
    }
    
    func copyAddressToPasteboard() {
        UIPasteboard.general.string = currentWallet?.address
    }
...
extension WalletModel {
    func fetchAssets() {
        ...
        let addr = currentWallet.address
        addressSocket.emit("get", ["scope": ["assets", "portfolio", "transactions"], "payload": ["address": addr, "currency": "usd"]])
